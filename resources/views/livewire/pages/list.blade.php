<div wire:key='{{ $employee->id }}'>
    <flux:modal.trigger name="show-employee-details-{{ $employee->id }}" class="cursor-pointer">
        <div
            class="bg-dark-200 border border-dark-400 p-6 rounded-2xl shadow-lg card-hover bg-gray-700 hover:bg-gray-800 hover:scale-105 transition-all duration-300 ease-in-out">
            <div class="flex justify-between items-start mb-4">
                <div class="flex flex-col">
                    <h2 class="text-lg font-semibold text-white">{{ $employee->name }}</h2>
                    <span class="text-xs text-gray-200">{{ $employee->position }}</span>
                    <span class="text-xs text-gray-200">{{ $employee->department }}</span>
                </div>
                <div class="p-2 rounded-lg bg-dark-300">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 text-accent-purple" viewBox="0 0 20 20"
                        fill="currentColor">
            ...
            <div class="mt-4 pt-4 border-t border-dark-300">
                <p class="text-gray-300 flex gap-2">
                    ...
                    @else
                        <span
                            class="font-medium text-white">₱{{ number_format($employee?->utangs?->amount, '2', '.', ',') }}
                        </span>
                    @endif
                </p>
            </div>
